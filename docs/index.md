---
title: Unicorns and Other Mythical Beasts
description: An interactive intelligent textbook about unicorns, mythical beasts, and the technologies that will improve them. A satirical exploration of AI hype, education resistance, and technology fantasies.
image: /img/cover.png
og:image: /img/cover.png
twitter:image: /img/cover.png
hide:
  - toc
---
<style>
.md-content__inner h1 {display: none !important;}
</style>

# Welcome

<figure markdown>
  ![Unicorns and Other Mythical Beasts](./img/cover.png){ width="100%" }
</figure>

Welcome to **Unicorns and Other Mythical Beasts: An Essential Guide to Creatures That Definitely Exist and the AI Technologies That Will Improve Them**.

This is a rigorous, evidence-adjacent exploration of mythical creatures and the parallel universe of artificial intelligence — a field that, like unicorns, promises magical solutions to all of humanity's problems and may or may not actually deliver. Through 19 chapters of satirical stories, interactive simulations, and graphic novels, you will develop the critical thinking skills necessary to distinguish between things that are real, things that are imaginary, and things that are "in beta."

## What You Will Find Here

- **19 Chapters** across 5 units — from the history of unicorns to the bestiary of vaporware
- **Interactive MicroSims** — browser-based simulations including population dynamics, hype cycle visualizations, and a vaporware field guide
- **12 Graphic Novels** — mythical beasts navigating workplace situations that will feel painfully familiar
- **Quizzes** — where the wrong answers are sometimes more true than the right ones
- **A Glossary** — defining terms you never knew you needed, with dictionary precision and zero irony
- **A Learning Graph** — visualizing 140+ concept dependencies across the course

## Who This Book Is For

This course is designed for anyone ages 14 and up who has ever been told "AI won't affect your job," attended a meeting about digital transformation, or wondered whether the metaverse was a real thing. No prior experience with unicorns, dragons, or artificial intelligence is required. A functioning sense of humor is strongly recommended.

## Getting Started

Start with [Chapter 1: A Brief and Totally Accurate History of Unicorns](chapters/01-history-of-unicorns/index.md), or explore the [Learning Graph](learning-graph/index.md) to see how concepts connect across chapters.
